...
def absolute_image(value):
    if re.match(r"https?://", value, flags=re.I):
        return value
    return f"{SITE}/{quote(value, safe='/')}"
...
def card(listing, prefix="../../"):
    ...
    return f'''<a class="listing-card" href="{prefix}listings/{listing['slug']}/"><img loading="lazy" src="{escape(image)}" alt="Front exterior of {escape(listing['street'])} in {escape(listing['city'])}, Tennessee"><div><strong>{escape(listing['price'])}</strong><span>{escape(listing['address'])}</span><span>{escape(details or listing['type'])}</span></div></a>'''
...
def write_listing_pages(listings):
    ...
    for item in listings:
        ...
        gallery = ''.join(f'<img src="{escape(url)}" alt="{escape("Front exterior" if index == 0 else f"Property view {index + 1}")} of {escape(item["street"])} in {escape(item["city"])}, Tennessee">' for index, url in enumerate(image_urls[:5]))
...
def write_robots_and_sitemap(listings, cities):
    urls = [SITE + "/", SITE + "/forsale.html", SITE + "/about.html", SITE + "/contact.html", SITE + "/events.html", SITE + "/amy-mclemore.html"]
    urls += [f"{SITE}/homes-for-sale/{slugify(city)}-tn/" for city in cities]
    urls += [f"{SITE}/listings/{item['slug']}/" for item in listings]
...
def main():
    ...
    write_robots_and_sitemap(listings, cities)
    print(f"Generated {len(cities)} city pages and {len(listings)} permanent listing pages.")
