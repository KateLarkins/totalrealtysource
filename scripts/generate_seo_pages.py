...
def parse_listings():
    ...
    for index, match in enumerate(cards):
        ...
        card = html[match.start():min(modal_start, next_card)]
        modal_end = next((candidate.start() for candidate in cards[index + 1:] if candidate.start() > modal_start), len(html))
...
def site_footer(prefix=""):
    return f'''<footer><strong>Total Realty Source</strong><span>117 South Main Street, Medina, TN 38355</span><a href="tel:17315749340">731-574-9340</a><a href="{prefix}contact.html">Contact our team</a><a href="{prefix}homes-for-sale/jackson-tn/">Jackson homes for sale</a><a href="{prefix}homes-for-sale/humboldt-tn/">Humboldt homes for sale</a><a href="{prefix}jackson-tn/new-construction/">New homes in Jackson</a><a href="{prefix}west-tennessee/land-acreage/">West Tennessee land and acreage</a><a href="{prefix}homes-for-rent/west-tennessee/">West Tennessee homes for rent</a></footer>'''
...
def write_city_pages(listings):
    ...
    for city in cities:
        ...
        local = [item for item in listings if item["city"].lower() == city.lower() and item["category"] == "homes" and "/month" not in item["price"].lower()]
        ...
        market_copy = {
            ...
            "Humboldt": "Browse active houses in Humboldt and compare current Gibson County listing prices, home features, photos, and locations. Total Realty Source can help buyers confirm availability and arrange a showing.",
            "Medina": "Explore houses for sale in Medina and the surrounding Gibson County market. Review active listing details and connect with a local Total Realty Source agent for current availability and neighborhood guidance.",
        }.get(city, f"Browse active houses for sale in {city}, Tennessee. Compare current listing prices, property details, photos, and locations with help from a West Tennessee real estate professional.")
        collection = {"@context":"https://schema.org","@type":"CollectionPage","name":title,"description":description,"url":canonical,"about":{"@type":"Place","name":f"{city}, Tennessee"}}
        item_list = {"@context":"https://schema.org","@type":"ItemList","name":f"Current homes for sale in {city}, Tennessee","numberOfItems":len(local),"itemListElement":[{"@type":"ListItem","position":position,"url":f"{SITE}/listings/{item['slug']}/","name":item["address"]} for position,item in enumerate(local,1)]}
        page = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(title)}</title><meta name="description" content="{escape(description)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="{canonical}"><meta property="og:type" content="website"><meta property="og:url" content="{canonical}"><meta property="og:title" content="{escape(title)}"><meta property="og:description" content="{escape(description)}"><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Bodoni+Moda:wght@500;700&family=Lora:wght@400;600;700&display=swap" rel="stylesheet"><style>{CSS}</style><script type="application/ld+json">{json.dumps([breadcrumb,collection,item_list])}</script></head><body>{site_header('../../')}<main class="shell"><nav class="crumbs"><a href="../../index.html">Home</a> / <a href="../../forsale.html">Homes for Sale</a> / {escape(city)}, TN</nav><h1>Homes for Sale in {escape(city)}, Tennessee</h1><p class="intro">{escape(market_copy)}</p><div class="category-links"><a href="../../forsale.html?type=homes&amp;location={quote(city + ', TN')}">Search all {escape(city)} homes</a><a href="../../contact.html">Ask a local agent</a></div><h2>Current houses for sale in {escape(city)}, TN</h2><div class="listing-grid">{''.join(card(item) for item in local) or '<p>No active houses are displayed in this city right now. Contact our team for recently added and upcoming options.</p>'}</div><h2>Buying a home in {escape(city)}</h2><p class="seo-copy">Use this page to compare active {escape(city)} homes by price, location, bedrooms, bathrooms, photos, and property details. Availability can change quickly. A Total Realty Source agent can confirm the latest MLS status, arrange a tour, and help you evaluate an offer without leaving this local search.</p><h2>Explore nearby West Tennessee housing markets</h2><div class="nearby">{''.join(f'<a href="../{slugify(other)}-tn/">{escape(other)} homes for sale</a>' for other in nearby)}</div></main>{site_footer('../../')}</body></html>'''
...
def write_jackson_specialty_pages(listings):
    ...
    for slug,title,description,matcher in pages:
        ...
        collection = {"@context":"https://schema.org","@type":"CollectionPage","name":title,"description":description,"url":canonical,"about":{"@type":"Place","name":"Jackson, Tennessee"}}
        intent_copy = {
            "new-construction": "New-construction inventory changes as builders release homes and lots. Even when no matching active listing is displayed, a local agent can help you check recently added homes, planned communities, build timelines, warranties, and available lots in the Jackson area.",
            "homes-with-land": "Compare acreage, lot size, access, utilities, outbuildings, and property use before scheduling a tour. Listing details should be confirmed through the current MLS record and appropriate inspections.",
        }.get(slug, "Compare current prices, photos, property details, and locations, then ask a local Total Realty Source agent to confirm availability and arrange a showing.")
        page = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(title)} | Total Realty Source</title><meta name="description" content="{escape(description)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="{canonical}"><meta property="og:type" content="website"><meta property="og:url" content="{canonical}"><meta property="og:title" content="{escape(title)}"><meta property="og:description" content="{escape(description)}"><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Bodoni+Moda:wght@500;700&family=Lora:wght@400;600;700&display=swap" rel="stylesheet"><style>{CSS}</style><script type="application/ld+json">{json.dumps([breadcrumb,collection,item_list])}</script></head><body>{site_header('../../')}<main class="shell"><nav class="crumbs"><a href="../../index.html">Home</a> / <a href="../../homes-for-sale/jackson-tn/">Jackson homes for sale</a> / {escape(title)}</nav><h1>{escape(title)}</h1><p class="intro">{escape(description)}</p><p class="seo-copy">{escape(intent_copy)}</p><div class="nearby">{links}</div><h2>Current matching properties</h2><div class="listing-grid">{''.join(card(item) for item in matches) or '<p>No matching active properties are displayed right now. Contact our team for upcoming or recently added options.</p>'}</div><p><a class="cta" href="../../contact.html">Ask a Jackson-area agent</a></p></main>{site_footer('../../')}</body></html>'''
        (folder / "index.html").write_text(page, encoding="utf-8")
        urls.append(canonical)
    return urls


def write_regional_intent_pages(listings):
    pages = [
        ("west-tennessee/land-acreage", "Land and Acreage for Sale in West Tennessee", "Browse active West Tennessee land, acreage, lots, and properties with room to spread out.", [item for item in listings if item["category"] == "land" or re.search(r"\b(acre|acres|acreage)\b", item["description"], re.I)]),
        ("homes-for-rent/west-tennessee", "Homes for Rent in West Tennessee", "View current West Tennessee residential rentals and connect with Total Realty Source for availability and showing information.", [item for item in listings if item["category"] == "homes" and ("/month" in item["price"].lower() or re.search(r"\b(for rent|for lease|lease term)\b", item["description"], re.I))]),
    ]
    urls = []
    for path, title, description, matches in pages:
        canonical = f"{SITE}/{path}/"
        folder = ROOT / path
        folder.mkdir(parents=True, exist_ok=True)
        breadcrumb = {"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Home","item":SITE},{"@type":"ListItem","position":2,"name":title,"item":canonical}]}
        collection = {"@context":"https://schema.org","@type":"CollectionPage","name":title,"description":description,"url":canonical}
        item_list = {"@context":"https://schema.org","@type":"ItemList","name":title,"numberOfItems":len(matches),"itemListElement":[{"@type":"ListItem","position":position,"url":f"{SITE}/listings/{item['slug']}/","name":item["address"]} for position,item in enumerate(matches,1)]}
        guidance = "For rental availability, lease terms, deposits, pet policies, and showing information, confirm the current details with Total Realty Source before applying." if path.startswith("homes-for-rent") else "Compare acreage, location, road access, utilities, zoning, improvements, and listing details before arranging a property tour."
        page = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(title)} | Total Realty Source</title><meta name="description" content="{escape(description)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="{canonical}"><meta property="og:type" content="website"><meta property="og:url" content="{canonical}"><meta property="og:title" content="{escape(title)}"><meta property="og:description" content="{escape(description)}"><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Bodoni+Moda:wght@500;700&family=Lora:wght@400;600;700&display=swap" rel="stylesheet"><style>{CSS}</style><script type="application/ld+json">{json.dumps([breadcrumb,collection,item_list])}</script></head><body>{site_header('../../')}<main class="shell"><nav class="crumbs"><a href="../../index.html">Home</a> / {escape(title)}</nav><h1>{escape(title)}</h1><p class="intro">{escape(description)} Listing availability changes frequently, so contact a local agent to confirm current status and schedule a showing.</p><p class="seo-copy">{escape(guidance)}</p><div class="category-links"><a href="../../forsale.html">Search all West Tennessee properties</a><a href="../../contact.html">Ask a local agent</a></div><h2>Current matching West Tennessee properties</h2><div class="listing-grid">{''.join(card(item) for item in matches) or '<p>No matching properties are displayed right now. Contact our team for recently added options.</p>'}</div><h2>More ways to search</h2><div class="nearby"><a href="../../homes-for-sale/jackson-tn/">Jackson homes for sale</a><a href="../../homes-for-sale/humboldt-tn/">Humboldt homes for sale</a><a href="../../homes-for-sale/medina-tn/">Medina homes for sale</a><a href="../../jackson-tn/new-construction/">New homes in Jackson</a></div></main>{site_footer('../../')}</body></html>'''
        (folder / "index.html").write_text(page, encoding="utf-8")
        urls.append(canonical)
    return urls
...
def main():
    ...
    specialty_urls += write_regional_intent_pages(listings)
